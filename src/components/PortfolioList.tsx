          ...
          <tbody className="divide-y divide-slate-700/50">
            {sortedPortfolio.map((item) => {
              ...
              const isPositive = gain >= 0;
                ...
                <tr
                  key={item.id}
                  className={`hover:bg-slate-700/20 transition-colors ${item.isRefreshing ? 'bg-slate-800/60 ring-2 ring-blue-400/30' : ''}`}
                >
                  ...
                  <td className="p-4 text-slate-300 font-medium w-24 whitespace-nowrap">{item.symbol}</td>
